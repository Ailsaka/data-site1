...
(function () {
  ...
  var temp_91 = new Array();
  var temp_31 = new Array();
  ...
  for(var i=0,len=missiles.length;i<len;i++){
    if(missiles[i] != 'K/AKD91'&&missiles[i] != 'Х-31П'){
        ...
        // 假的 放入两个temp
        if(missiles[i]=='K/AKD91'){
            temp_91.push(missiles[i]);
            for(var j=0, len1=reasons.length; j<len1; j++){
                if(reasons[j]=='设计不足'){
                    temp_91.push(9);
            ...
            temp_31.push(missiles[i]);
            for(var j=0, len1=reasons.length; j<len1; j++){
                    temp_31.push( barDataAllSeriesForMR[missiles[i]][reasons[j]] );
            };
        };
    };
  };
  for(var ri=0,len2=reasons.length;ri<len2;ri++){
    option.series.push(            { type: 'bar', }        );
  };

  //加入91 31
  option.dataset.source.push(temp_91);
  option.dataset.source.push(temp_31);
